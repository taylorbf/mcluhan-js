...
var Hack = module.exports = function(params) {
  ...
  this.elementsLoaded = 0;
	for (var i=0;i<this.element.length;i++) {
		...
		this.element[i].onload = function() {
      this.playVine()
...
Hack.prototype.playVine = function() {
  this.elementsLoaded++;
  if (this.elementsLoaded >= this.element.length) {
    for (var i=0;i<this.element.length;i++) {
      this.element[i].contentWindow.postMessage("play","*")
    }
    this.elementsLoaded = 0
  }
	return this
...
Hack.prototype.load = function(src) {
	...
	if (src.indexOf("http")==0) {
		...
		/*if (src.indexOf("vine")>=0) {
			for (var i=0;i<this.spaces.length;i++) {
				this.element[i].contentWindow.postMessage("play","*")
			}	
			//next: check if vines autoplay!
		} */
