 ...
 * Remove all media elements and content from all windows in this wall
 */
Wall.prototype.emptyAll = function() {
 ...
 * @param  {String} x  x offset
 * @param  {String} y  y offset
 * @return {Wall}
 */
Wall.prototype.wander = function(x,y) {
	for (var i=0;i<this.elements.length;i++) {
		var newx = this.elements[i].element.screenX + x
		var newy = this.elements[i].element.screenY + y
		if (newx >= m.stage.x + m.stage.w) {
			newx = m.stage.x
		}
		if (newy >= m.stage.y + m.stage.h) {
			newy = m.stage.y
		}
		this.elements[i].move(newx,newy)
	}
	return this
}
